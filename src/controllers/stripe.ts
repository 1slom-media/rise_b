...
const createOrder = async (customer, data) => {
  try {
    ...
    const carts = await AppDataSource.getRepository(CartEntity).find({
      relations: ["products", "products.parametrs", "company", "user"],
    });
    const cartFilter = carts.filter((cart) => cart?.user?.id === +user);
    ...
    for (const cart of cartFilter) {
      ...
      const updatedProduct = await AppDataSource.getRepository(
        ProductsEntity
      ).findOne({ where: { id: productId }, relations: ["parametrs"] });

      for (const indeks of cart.indeks) {
        const orderColor = indeks["color"];
        const matchingParam = updatedProduct?.parametrs.find(
          (param) => param?.color === orderColor
        );
        const parametrs = await AppDataSource.getRepository(
          ParametrsEntity
        ).findOne({ where: { id: matchingParam?.id } });
